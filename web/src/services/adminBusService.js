...
export const adminBusService = {
    ...
    async getBusDeleteImpact(busId) {
        try {
    ...
    async deactivateBus(busId) {
        try {
            ...
            if (tripError)
                throw new Error(`Bus deactivated, but future trips could not be closed: ${tripError.message}`);
    ...
    async createBus(bus) {
        try {
            ...
            if (bus.routeId) {
                ...
                if (tripError)
                    throw new Error(`Bus created, but route assignment failed: ${tripError.message}`);
            }
        ...
        try {
            ...
            if (updates.routeId !== undefined) {
                ...
                if (existingTrips && existingTrips.length > 0) {
                    ...
                    if (tripUpdateError)
                        throw new Error(`Bus updated, but route assignment failed: ${tripUpdateError.message}`);
                }
                    ...
                    if (tripInsertError)
                        throw new Error(`Bus updated, but route assignment failed: ${tripInsertError.message}`);
                }
            }
    ...
    async deleteBus(busId) {
        try {
            ...
            const { error: directBookingUpdateError } = await supabase
                ...
                .update({ bus_id: null })
                .eq('bus_id', busId);
            if (directBookingUpdateError)
                throw new Error(`Could not detach bus bookings: ${directBookingUpdateError.message}`);
            if (tripIds.length > 0) {
                const { error: tripBookingUpdateError } = await supabase
                    .from('bookings')
                    .update({ trip_id: null, bus_id: null })
                    .in('trip_id', tripIds);
                if (tripBookingUpdateError)
                    throw new Error(`Could not detach trip bookings: ${tripBookingUpdateError.message}`);
                const { error: tripDeleteError } = await supabase
                    .from('trips')
                    .delete()
                    .eq('bus_id', busId);
                if (tripDeleteError)
                    throw new Error(`Could not delete bus trips: ${tripDeleteError.message}`);
            }
...
function mapBus(raw) {
    ...
    if (raw.trips && Array.isArray(raw.trips)) {
        const activeTrip = raw.trips.find((trip) => trip.status === 'scheduled' || trip.status === 'ongoing');
        if (activeTrip)
            activeRouteId = activeTrip.route_id;
    }
    return {
        id: raw.id,
